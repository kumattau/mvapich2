...
might want to use the \texttt{--ncpus} argument described in Section~\ref{sec:smps}.
...
specific interface by using the \emph{---ifhn} (interface-hostname) option, e.g.:
\begin{verbatim}
...
named \emph{MPD\_USE\_ROOT\_MPD}.  This option may either be set as an environment
...
user except that root's configuration file is in /etc/mpd.conf (note that
...
kill -9 PID_OF_MPD
\end{verbatim}
where you have obtained the PID\_OF\_MPD by some means such as the ps command.
...
\begin{verbatim}
    n2 $  mpd -h n1 -p 6789 &
\end{verbatim}
where 6789 is the listeneing port on n1 (from mpdtrace above).
Now try:
\begin{verbatim}
    n2 $  mpdtrace -l
\end{verbatim}
