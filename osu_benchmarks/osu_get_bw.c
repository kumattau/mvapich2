/*
 * Copyright (C) 2003-2005 the Network-Based Computing Laboratory
...
int main (int argc, char *argv[])
{
    ...
    if (myid == 0) {
        fprintf (stdout,
                 "# OSU MPI2 MPI_Get Bandwidth Test (Version 2.1)\n");
    ...
    for (size = 1; size <= MAX_SIZE; size *= 2) {
        ...
        if (myid == 0) {
            MPI_Win_create (s_buf, size, 1, MPI_INFO_NULL,
                            MPI_COMM_WORLD, &win);
            destrank = 1;
            ...
            for (i = 0; i < WARMUP; i++) {
                ...
                MPI_Get ((char *) ((unsigned long) r_buf + i * size),
                         size, MPI_CHAR, 1, 0, size, MPI_CHAR, win);
        ...
        if (myid == 0) {
            ...
            for (i = 0; i < loop; i++) {
                ...
                for (j = 0; j < WINDOW_SIZE; j++) {
                    MPI_Get ((char *) ((unsigned long) r_buf + j * size),
                             size, MPI_CHAR, 1, 0, size, MPI_CHAR, win);
                } MPI_Win_complete (win);
            } 
