#define BENCHMARK "OSU MPI%s One Sided MPI_Compare_and_Swap with Flush Latency Test"
/*
...
main (int argc, char *argv[])
{
    ...
    uint64_t    *s_buf, *r_buf, *t_buf, *c_buf;
    ...
    if (allocate_memory(&s_buf, &r_buf, &t_buf, &c_buf, myid)) {
    ...
    if (myid == 1) { 
        ...
        for (i = 0; i < (loop + skip); i++) {
            ...
            MPI_Compare_and_swap (s_buf, c_buf, t_buf, MPI_LONG_LONG, 0, 0, win);                      
            MPI_Win_flush(0, win);
        }
                 ...
                 uint64_t ** cbuf, int rank)
{
    ...
    switch (rank) {
        case 0: 
            ...
            *cbuf = align_buffer(c_buf_original, align_size);
            break;
        case 1:
            *sbuf = align_buffer(s_buf_original, align_size);
            *rbuf = align_buffer(r_buf_original, align_size);
