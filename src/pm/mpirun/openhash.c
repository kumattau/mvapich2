...
void	dump_hash_table( hash_table_t* ht )
{
	...
	for(i=0; i<ht->length; i++)
	{
		...
		if( mv2_list_empty( &bkt->list) )	continue;
...
ckpt_file_t*	hash_table_create_record(hash_table_t* ht, char* fname )
#else
...
*/
#if	HASH_FROM_FILENAME
ckpt_file_t*	hash_table_get_record(hash_table_t* ht, char* fname, int is_last_chunk )
#else
...
int hash_table_put_record(hash_table_t* ht, ckpt_file_t* cfile, int is_release )
{
	...
	dbg("Enter:: cfile %s: ref = %d, fd=%d, can-rel=%d, has-last-chunk=%d\n", cfile->filename, 
        atomic_read(&cfile->ref), cfile->fd, cfile->can_release, cfile->has_last_chunk );
	...
	dbg("\n*** Now, free -- (%s)(%d,%d) : total chunk_num %d, pending chunks %d, has_last_chunk %d, adv-size=%d\n\n", 
				cfile->filename, cfile->ckpt_id, cfile->proc_rank,  cfile->rcv_total_chunk_num, 
				cfile->rcv_pending_RR_num, cfile->has_last_chunk, cfile->adv_size );
			...
			&& cfile->has_last_chunk )
	{
		...
		printf("\tRelease ckpt-%d-rank-%d : chunk_num %d, chunk_to_write %d, has_last_chunk %d, size=%d\n", 
				cfile->ckpt_id, cfile->proc_rank, 
				cfile->chunk_num, cfile->chunk_to_write, cfile->has_last_chunk,
				cfile->write_size );
		/// close the ckpt-file
...
**/
hash_bucket_t* htbl_find_lock_bucket(hash_table_t* ht, char* fname, int namelen)
{
	#if	HASH_FROM_FILENAME
