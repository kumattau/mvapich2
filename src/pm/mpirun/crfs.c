...
/////////
static int		sim_iothr_num = 8;
static pthread_t	sim_iothr[16]; // 
////////////////////
 ...
 * Introduced in version 2.3
 */
int crfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset,
...
int crfs_getattr(const char *path, struct stat *statbuf)
{
	...
	if( crfs_mode == MODE_MIG )
	{ // at srv-side: proc-migration needs the file-size to be large enough
		// to restart a proc
...
int crfs_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    ...
    /////////////////////////////////////
#ifdef CKPT_MIG
	//parse the ckpt-id, proc-rank, then lookup the hash-table for this cfile;
	int ckptid=0, procrank=0;
	...
	while( to_read > 0 )
	{
		...
		if( chunk )
		{
			if( offset < chunk->offset + chunk->curr_pos ){ // this should never happen
				error( "read offset %d < chunk->offset %d + curr_pos %d\n", 
			...
			if( offset >=  (chunk->offset + chunk->size)  ){ // this should never happen...
				error("read offset %d > curr_chunk (%d+%d)\n", offset,
		...
		{  // the curr_chunk is NULL, need to get a chunk
			...
			if( cfile->curr_chunk == NULL )// the ckpt-file is completed, ret
				break;
			if( cfile->curr_chunk->size==0 )
			{	// a dummy chunk: signify the end of file				
...
int crfs_flush(const char *path, struct fuse_file_info *fi)
{
	...
	if( !bkt )
	{ // the target file has already been cleaned from hash-tbl
...
int crfs_release(const char *path, struct fuse_file_info *fi)
{
	...
	if( crfs_mode == MODE_MIG ) // at mig-FS serv side: for migration
	{
		...
		{	/// clear all remaining chunk-bufs
