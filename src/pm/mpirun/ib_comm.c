...
long cli_rdmabuf_size = 8*1024*1024; // 128*1024*1024;
long srv_rdmabuf_size = 8*1024*1024; // 256*1024*1024;


int sendslot_size = 64;
int recvslot_size = 64;
int rdmaslot_size = (1024*1024*1); // *1024*2;
...
////////////////
static struct ibv_cq  *share_cq = NULL;   // connections in one HCA share this same CQ
static struct ibv_comp_channel *share_comp_channel = NULL;

static atomic_t cq_use = ATOMIC_INIT(0); // how many connections are using this CQ
static atomic_t comp_channel_use = ATOMIC_INIT(0); 
...
void	dump_ib_packet( struct ib_packet* pkt)
{
	...
	switch( pkt->command ){
		...
		case rqst_RR:
			cmd = "rqst_RR";
			printf("ib-packet:  cmd = \"%s\" \n", cmd);
			printf("    rbuf = %p, rbuf-id=%d, size = %d\n", 
...
{
	if( atomic_sub_and_test( 1, &hca->ref ) )
	{	// no connection is referring it, release the HCA
...
/*	g_device = ret>1? g_device: 0;
	...
	// get CQ
	hca->cq = ibv_create_cq( hca->context, 
                hca->rx_depth+1, // rx_depth+1,  /* CQE depth */
...
int	ib_HCA_destroy(struct ib_HCA* hca)
{
	int i, ret;
...
int	ib_connection_buf_init(struct ib_HCA* hca, int sendsize, int recvsize, int rdmasize)
{
	...
	hca->max_rq_wqe = ( hca->recv_buf->num_slot < hca->rx_depth ) ? 
			hca->recv_buf->num_slot : hca->rx_depth; 
	...
	if( rdmasize > 0 ){	
		...
		hca->rdma_buf->mr = ibv_reg_mr( hca->pd, hca->rdma_buf->addr, rdmasize, IBV_ACCESS_LOCAL_WRITE 
...
{	
	...
	dbg("post RR: qp %d(qpn %x) (%s) %ld@%ld, lbuf-slot %d, rmt-addr %p, rmt-bufid %d, rkey 0x%x, ret %d\n", 
		qpidx, conn->qp[qpidx]->qp_num, 
		rrpkt->RR.filename, rrpkt->RR.size, rrpkt->RR.offset, slot, 
		rrpkt->RR.raddr, rrpkt->RR.rbuf_id, rrpkt->RR.rkey,	i );
...
int inline	ib_connection_send_RR_rqst(struct ib_connection* conn, int qpidx, 
	int rbufid,	int rprocid, int rckptid, int size, int offset, int is_last_chunk )
{
	struct ib_HCA* hca = conn->hca;
...
int ib_connection_send_chunk_RR_rqst(struct ib_connection* conn, ckpt_file_t* cfile, ckpt_chunk_t* chunk, void* arg)
{
	...
	cfile->filename, chunk->curr_pos, chunk->offset, 
		chunk->bufid, hca->rdma_buf->free_slots);
	...
	{	
		...
		case	IBV_WC_RDMA_READ: // finished a RDMA-READ, only on server
			...
			dbg("finished RDMA-READ (%s) %ld@%ld,rbufid=%d, lbufid=%d\n", //
				...
				pkt->RR.rbuf_id, pkt->RR.lbuf_id ); // pkt->RR.laddr );
			//dump_ib_packet(pkt);

			/// wake up the iothread who is waiting for this RR to complete
			sem_t* sem = (sem_t*) pkt->RR.larg1;
			if( sem )	sem_post( sem ); // wake up the iothread, who is waiting for the RR to complete
			else{
		...
		case	IBV_WC_RECV:  // receive an inbound message
			...
			if( is_server(server) ) 
			{
				...
				*/	
					...
					workqueue_enqueue3( tpool->queue, pkt, sizeof(*pkt), arg1, arg2, 
						(unsigned long)cfile );
...
int ib_server_loop(struct ib_connection* connarray, struct thread_pool* tp )
{
	...
	//while( ! all_connection_finished(connarray, atomic_read( &hca->ref) ) )
	{
		...
		do
		{
		    ...
		    //dbg(" Poll get %d cqe\n", ne);
   			if( ne == 0 )	break;  // no cqe in CQ, can break the do-while   
	...
	while( ! all_connection_finished(connarray, atomic_read( &hca->ref) ) )
	{
		...
		do
		{
			...
			for(i=0; i<ne; i++)
			{
				wc = &wc_array[i];
				//dump_wc( wc_array+i );
				if( find_qp_from_conn_array( wc->qp_num, connarray, atomic_read(&hca->ref), 
						&connidx, &qpidx ) != 0 ){
					error("Error:: Fail to find connidx, qpidx for qpn %d\n", wc->qp_num );
					//dump_wc(wc);
				} else {	
...
*/
int	inline ib_connection_send_terminate_rqst(struct ib_connection* connarray)
{
	struct ib_HCA* hca = connarray[0].hca;
...
*/
int pass_hash_table( hash_table_t* ht )
{
	ht_cfile = ht;
}
