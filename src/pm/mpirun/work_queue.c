...
void	dump_queue( struct work_queue* wq )
{
	...
	printf(" qlength = %d, head=%d, tail=%d\n", wq->qlength, wq->head, wq->tail);
...
*/
inline int	workqueue_enqueue3(struct work_queue* wq, void* elem, int elem_size, 
...
*/
inline int	workqueue_enqueue(struct work_queue* wq, void* elem, int elem_size, 
	unsigned long arg1, unsigned long arg2 )
{
	return	workqueue_enqueue3( wq, elem, elem_size, arg1, arg2, NULL );
...
void	dump_work_elem( struct work_elem* we )
{
	...
	printf("elem: rbuf_id=%d, rprocid=%d, rckptid=%d, size=%d, off=%d, lbuf_id=%d\n", 
		pkt->RR.rbuf_id, pkt->RR.rprocid, pkt->RR.rckptid, pkt->RR.size, pkt->RR.offset,
		pkt->RR.lbuf_id);
