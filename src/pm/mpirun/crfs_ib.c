...
int	ibsrv_main_entry(void* p)
{
	...
	if( pthread_create( &listen_thr, NULL, ibsrv_listen_port, (void*)port ) != 0 ){
...
void* ibsrv_ioprocess(void* arg)
{
	...
	/////////
	if( myid < 0 ){ // Error!!
		error("Error::  mytid = %u, fail to find id\n", mytid);
		goto err_out_1;
	} else {
		printf(" ioprocess %u, id = %d\n", mytid, myid);
	}
...
{
	int	port = (int)((unsigned long)(arg));
	char	msg[64];
	int srv_sock;
...
int	ibcli_end_mig(mig_info_t* minfo)
{
	int i;
...
the aggre-write bw is only ~200MB/s !!!!!!!!!!!	 */
		...
		if( mig_role == ROLE_MIG_SRC )
		{	/// will perform RDMA to mig-tgt node
		...
		{	
			...
			if( ret < 0 ){
				printf("[iot_%d] write (%s) (%d@%d) ret=%d, lbuf=%d, is-last-chunk=%d\n", 
				myid, cfile->filename, chunk->curr_pos, chunk->offset, ret, chunk->bufid, chunk->is_last_chunk);
...
chunk_done:	
		if( cfile->can_release &&  !chunk->is_last_chunk 			
			&& (chunk->offset + chunk->curr_pos==cfile->adv_size) )// this is the last chunk
		{
