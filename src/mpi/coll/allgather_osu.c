        ...
        if (is_homogeneous) {
            ...
            while (mask < comm_size) {
                ...
                if (dst < comm_size) {
                    mpi_errno =
                        MPIC_Sendrecv_ft(((char *) recvbuf + send_offset),
                                         ...
                                         dst, MPIR_ALLGATHER_TAG, comm, &status,
                                         errflag);
                    if (mpi_errno) {
                ...
                if (dst_tree_root + mask > comm_size) {
                    ...
                    /* FIXME: saving an MPI_Aint into an int */
                    offset =
                        recvcount * (my_tree_root + mask) * recvtype_extent;
                    tmp_mask = mask >> 1;
            ...
            if (sendbuf != MPI_IN_PLACE) {
                ...
                MPIR_Pack_impl(((char *) recvbuf + recvtype_extent * rank),
                               recvcount, recvtype, tmp_buf, tmp_buf_size,
                               &position);
            }
