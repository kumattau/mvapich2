...
static int MPIR_Reduce_scatter_noncomm_MV2 (
    void *sendbuf,
    void *recvbuf,
    int *recvcnts,
...
int MPIR_Reduce_scatter_intra_MV2 ( 
    void *sendbuf, 
    void *recvbuf, 
    int *recvcnts, 
    MPI_Datatype datatype, 
...
*/
    ...
    if (!is_commutative) {
        ...
        if (pof2 == comm_size && is_block_regular) {
            ...
            mpi_errno = MPIR_Reduce_scatter_noncomm_MV2(sendbuf, recvbuf, recvcnts, datatype, op, comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = TRUE;
                MPIU_ERR_SET(mpi_errno, MPI_ERR_OTHER, "**fail");
...
/* not declared static because a machine-specific function may call this one in some cases */
int MPIR_Reduce_scatter_inter_MV2 ( 
    void *sendbuf, 
    void *recvbuf, 
    int *recvcnts, 
    MPI_Datatype datatype, 
...
*/
    ...
    if (comm_ptr->is_low_group) {
        ...
        mpi_errno = MPIR_Reduce_inter_MV2(sendbuf, tmp_buf, total_count, datatype, op,
                        ...
                        MPI_Datatype datatype, MPI_Op op, MPID_Comm *comm_ptr, int *errflag)
{
    ...
    if (comm_ptr->comm_kind == MPID_INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIR_Reduce_scatter_intra_MV2(sendbuf, recvbuf, recvcnts,
                                              datatype, op, comm_ptr, errflag);
        if (mpi_errno) MPIU_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno = MPIR_Reduce_scatter_inter_MV2(sendbuf, recvbuf, recvcnts,
                                              datatype, op, comm_ptr, errflag);
        if (mpi_errno) MPIU_ERR_POP(mpi_errno);
    }
