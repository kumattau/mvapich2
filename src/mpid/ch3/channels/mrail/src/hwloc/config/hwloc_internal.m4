...
AC_DEFUN([HWLOC_DEFINE_ARGS],[
    ...
    AC_ARG_ENABLE([libxml2],
                  AS_HELP_STRING([--disable-libxml2], 
		                 [Do not use libxml2 for XML support, use a custom minimalistic support]),,enable_libxml2=no)
    ...
    AC_ARG_ENABLE([libpci],
		  AS_HELP_STRING([--enable-libpci],
				 [Use libpci for PCI support. Note that hwloc may be tainted by the pciutils GPL license.]),,enable_libpci=no)
