...
static void clear_cache(void)
{
    ...
    if (NULL != map_processor_id_to_tuple) {
        MPIU_Free(map_processor_id_to_tuple);
        map_processor_id_to_tuple = NULL;
    }
    if (NULL != map_tuple_to_processor_id) {
        MPIU_Free(map_tuple_to_processor_id);
...
static void load_cache(const char *sysfs_mount)
{
    ...
    map_processor_id_to_tuple = MPIU_Malloc(sizeof(tuple_t) * 
                                       (max_processor_id + 2));
    if (NULL == map_processor_id_to_tuple) {
        clear_cache();
        return;
    ...
    /* Now that we know the max number of sockets, allocate some
       arrays */
    max_core_id = MPIU_Malloc(sizeof(int) * (max_socket_id + 1));
    if (NULL == max_core_id) {
        clear_cache();
        return;
    }
    num_cores = MPIU_Malloc(sizeof(int) * (max_socket_id + 1));
    if (NULL == num_cores) {
        clear_cache();
        return;
    }
    ...
    cores_on_sockets = MPIU_Malloc(sizeof(PLPA_NAME(cpu_set_t)) * 
                              (max_socket_id + 1));
    ...
    /* Now go through and build the map in the other direction:
       ...
       value). */
    map_tuple_to_processor_id = MPIU_Malloc(sizeof(tuple_t *) *
                                       ((max_socket_id + 1) *
                                        (max_core_id_overall + 1)));
    if (NULL == map_tuple_to_processor_id) {
        clear_cache();
...
int PLPA_NAME(map_to_processor_id)(int socket, int core, int *processor_id)
{
    ...
    if (socket < 0 || socket > max_socket_id ||
        core < 0 || core > max_core_id[socket]) {
        return ENOENT;
    }
    ...
    ret = map_tuple_to_processor_id[socket * (max_core_id_overall + 1) +
                                    core]->processor_id;
    if (-1 == ret) {
        return ENOENT;
    }
