...
extern "C" void pack_subarray( void *dst, void *src, int nx, int ny, int nz, int sub_nx, int sub_ny, int sub_nz, int h_x, int h_y, int h_z, int el_size, cudaStream_t stream) 
{
    int BLOCK_SIZE_X = 16;
    int BLOCK_SIZE_Y = 8;
    int BLOCK_SIZE_Z = 8;
    ...
    if (el_size == 4) {
        pack_subarray_float<<<dimgrid, dimblock, 0, stream>>>((float *) dst, (float *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    } else if (el_size == 1) {
        pack_subarray_char<<<dimgrid, dimblock, 0, stream>>>((char *) dst, (char *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    } else if (el_size == 8) {
        pack_subarray_double<<<dimgrid, dimblock, 0, stream>>>((double *) dst, (double *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    }
}

extern "C" void unpack_subarray( void *dst, void *src, int nx, int ny, int nz, int sub_nx, int sub_ny, int sub_nz, int h_x, int h_y, int h_z, int el_size, cudaStream_t stream)
{
    int BLOCK_SIZE_X = 16;
    int BLOCK_SIZE_Y = 8;
    int BLOCK_SIZE_Z = 8;
    ...
    if (el_size == 4) {
        unpack_subarray_float<<<dimgrid, dimblock, 0, stream>>>((float *) dst, (float *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    } else if (el_size == 1) {
        unpack_subarray_char<<<dimgrid, dimblock, 0, stream>>>((char *) dst, (char *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    } else if (el_size == 8) {
        unpack_subarray_double<<<dimgrid, dimblock, 0, stream>>>((double *) dst, (double *) src, nx, ny, nz, sub_nx, sub_ny, sub_nz, h_x, h_y, h_z );
    }
...
extern "C" void pack_unpack_vector_kernel( void *dst, int dpitch, void *src, int spitch, int width, int height, cudaStream_t stream)
{
    ...
    if ( rdma_cuda_thread_ysize != 0 ){
		    BLOCK_SIZE_Y = rdma_cuda_thread_ysize;
	  }else{
		switch ( elems ){
      case 1:
			  BLOCK_SIZE_Y = 1;
  ...
  if ( rdma_cuda_thread_blk_size != 0 ){
		BLOCK_THREAD_SIZE = ( (rdma_cuda_thread_blk_size < 1024) ? rdma_cuda_thread_blk_size : 1024);
	}
	else{
