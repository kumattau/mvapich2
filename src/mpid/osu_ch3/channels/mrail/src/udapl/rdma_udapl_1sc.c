...
void
MPIDI_CH3I_RDMA_post (MPID_Win * win_ptr, int target_rank)
{
    char *remote_address;
    char *origin_addr;
...
Consume_signals (MPID_Win * winptr, aint_t expected)
{
          ...
          if (ret == DAT_SUCCESS)
            {
                ...
                assert (event.event_data.dto_completion_event_data.status ==
                        DAT_DTO_SUCCESS);
                assert (event.event_data.dto_completion_event_data.
                        user_cookie.as_64 != -1);
  ...
  fn_exit:
    ASSERT (event.event_data.dto_completion_event_data.user_cookie.as_64 ==
            expected || expected == 0);
