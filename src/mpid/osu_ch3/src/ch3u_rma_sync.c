...
static int MPIDI_CH3I_Send_rma_msg(MPIDI_RMA_ops * rma_op,
				   MPID_Win * win_ptr,
...
static int MPIDI_CH3I_Recv_rma_msg(MPIDI_RMA_ops * rma_op,
				   MPID_Win * win_ptr,
				   MPI_Win source_win_handle,
				   MPI_Win target_win_handle,
				   MPIDI_RMA_dtype_info * dtype_info,
...
#   endif
	...
	if (win_ptr->fall_back != 1) {
		...
		(int *) malloc((comm_size - 1) * sizeof(int));
	     ...
	     * blocking call */
	    MPIDI_CH3I_RDMA_start(win_ptr, comm_size - 1,
				  ranks_in_win_grp);
	...
	MPIU_CHKLMEM_MALLOC(rma_target_proc, int *,
			    comm_size * sizeof(int), mpi_errno,
			    "rma_target_proc");
	for (i = 0; i < comm_size; i++)
	    rma_target_proc[i] = 0;
	...
	if (total_op_count != 0) {
	    ...
	    MPIU_CHKLMEM_MALLOC(dtype_infos, MPIDI_RMA_dtype_info *,
				total_op_count *
				sizeof(MPIDI_RMA_dtype_info), mpi_errno,
				"dtype_infos");
	    MPIU_CHKLMEM_MALLOC(dataloops, void **,
				total_op_count * sizeof(void *), mpi_errno,
				"dataloops");
	...
	MPIR_Nest_incr();
	mpi_errno =
	    NMPI_Reduce_scatter(MPI_IN_PLACE, rma_target_proc, recvcnts,
	...
	DEBUG_PRINT
	    (stderr, "[fence] origin counter %d, commsize %d, rma_target_proc %d,"
	     "now counter %d\n", win_ptr->my_counter, comm_size,
	     rma_target_proc[0],
	     win_ptr->my_counter - comm_size + rma_target_proc[0]);
	win_ptr->my_counter =
	    win_ptr->my_counter - comm_size + rma_target_proc[0];
	...
	while (curr_ptr != NULL) {
	    ...
	    target_win_handle =
		win_ptr->all_win_handles[curr_ptr->target_rank];
				   ...
				   MPID_Request ** request)
{
    ...
    /* OSU-MPI2 moves place of get vc pointer */
    MPID_Comm_get_ptr(win_ptr->comm, comm_ptr);
	...
	DEBUG_PRINT("[%d]set field for accm %d, vc %p, vc->issued %d, source win %p, win null %p\n",
            win_ptr->my_id, accum_pkt->rma_issued, vc, vc->rma_issued, source_win_handle,
		MPI_WIN_NULL);
    }
...
*/
	...
	if (!MPIDI_CH3_Eager_ok(vc, total_length)) {
...
int MPIDI_Win_post(MPID_Group * group_ptr, int assert, MPID_Win * win_ptr)
{
	...
	MPIU_CHKLMEM_MALLOC(ranks_in_post_grp, int *,
			    post_grp_size * sizeof(int), mpi_errno,
			    "ranks_in_post_grp");
	...
	for (i = 0; i < post_grp_size; i++) {
		...
		{
		    mpi_errno =
			NMPI_Send(&i, 0, MPI_INT, dst, 100, win_ptr->comm);
		    if (mpi_errno) {
			MPIU_ERR_POP(mpi_errno);
		    }
		}
...
int MPIDI_Win_complete(MPID_Win * win_ptr)
{
    ...
    MPIU_CHKLMEM_MALLOC(ranks_in_start_grp, int *,
			start_grp_size * sizeof(int), mpi_errno,
			"ranks_in_start_grp");
...
/* End of OSU-MPI2 */
    ...
    MPIU_CHKLMEM_MALLOC(requests, MPID_Request **,
			(total_op_count +
			 start_grp_size) * sizeof(MPID_Request *),
			mpi_errno, "requests");
...
int MPIDI_Win_lock(int lock_type, int dest, int assert, MPID_Win * win_ptr)
{
	...
	if (MPIDI_CH3I_Try_acquire_win_lock(win_ptr, lock_type) == 0) {
		...
		if (mpi_errno != MPI_SUCCESS) {
			...
			MPIR_Err_create_code(mpi_errno, MPIR_ERR_FATAL,
					     ...
					     MPI_ERR_OTHER, "**fail",
					     "**fail %s",
					     "making progress on rma messages failed");
		    goto fn_exit;
		}
		/* --END ERROR HANDLING-- */
	    }
	...
	MPIU_CHKPMEM_MALLOC(new_ptr, MPIDI_RMA_ops *,
			    sizeof(MPIDI_RMA_ops), mpi_errno,
			    "RMA operation entry");
...
int MPIDI_Win_unlock(int dest, MPID_Win * win_ptr)
{
	...
	mpi_errno =
	    MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, FCNAME,
				 __LINE__, MPI_ERR_OTHER, "**rmasync", 0);
	goto fn_exit;
    }
	...
	if (mpi_errno != MPI_SUCCESS) {
		...
		MPIR_Err_create_code(mpi_errno, MPIR_ERR_FATAL, FCNAME,
				     __LINE__, MPI_ERR_OTHER, "**fail",
				     "**fail %s",
				     "sending the rma message failed");
	    goto fn_exit;
	}
	/* --END ERROR HANDLING-- */
	...
	mpi_errno =
	    MPIDI_CH3I_Do_passive_target_rma(win_ptr,
					    ...
					    int *wait_for_rma_done_pkt)
{
	...
	*wait_for_rma_done_pkt = 0;
    ...
    MPIU_CHKLMEM_MALLOC(requests, MPID_Request **,
			nops * sizeof(MPID_Request *), mpi_errno,
			"requests");
    MPIU_CHKLMEM_MALLOC(dtype_infos, MPIDI_RMA_dtype_info *,
			nops * sizeof(MPIDI_RMA_dtype_info), mpi_errno,
			"dtype_infos");
...
{
    int mpi_errno =
	MPI_SUCCESS, lock_type, origin_dt_derived, iov_n, iovcnt;
	...
	MPIDI_Pkt_init(lock_accum_unlock_pkt,
