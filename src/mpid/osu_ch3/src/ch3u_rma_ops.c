...
int MPIDI_Win_create(void *base, MPI_Aint size, int disp_unit,
		     ...
		     MPID_Win ** win_ptr, MPIDI_RMAFns * RMAFns)
{
    ...
    *win_ptr = (MPID_Win *) MPIU_Handle_obj_alloc(&MPID_Win_mem);
    ...
    MPIU_CHKPMEM_MALLOC((*win_ptr)->base_addrs, void **,
			comm_size * sizeof(void *), mpi_errno,
			"(*win_ptr)->base_addrs");

    MPIU_CHKPMEM_MALLOC((*win_ptr)->disp_units, int *,
			comm_size * sizeof(int), mpi_errno,
			"(*win_ptr)->disp_units");

    MPIU_CHKPMEM_MALLOC((*win_ptr)->all_win_handles, MPI_Win *,
			comm_size * sizeof(MPI_Win), mpi_errno,
			"(*win_ptr)->all_win_handles");

    MPIU_CHKPMEM_MALLOC((*win_ptr)->pt_rma_puts_accs, int *,
			comm_size * sizeof(int), mpi_errno,
			"(*win_ptr)->pt_rma_puts_accs");
    for (i = 0; i < comm_size; i++)
	(*win_ptr)->pt_rma_puts_accs[i] = 0;
    ...
    MPIU_CHKLMEM_MALLOC(tmp_buf, MPI_Aint *,
			3 * comm_size * sizeof(MPI_Aint), mpi_errno,
			"tmp_buf");
	...
	(*win_ptr)->all_win_handles[i] = (MPI_Win) tmp_buf[3 * i + 2];
        ...
        if (comm_ptr->comm_kind != MPID_INTRACOMM)
            ...
            (*win_ptr)->fall_back = 0;
            MPIDI_CH3I_RDMA_win_create(base, size, comm_size, rank, win_ptr,
                                   comm_ptr);
        }
#endif
        /* end of OSU-MPI2 */
...
{
    int mpi_errno =
	MPI_SUCCESS, total_pt_rma_puts_accs, i, *recvcnts, comm_size;
    MPID_Comm *comm_ptr;
    MPIU_CHKLMEM_DECL(1);
	      ...
	      MPID_Win * win_ptr)
{
    ...
    if (target_rank == rank) {
	mpi_errno =
	    MPIR_Localcopy(origin_addr, origin_count, origin_datatype,
			   (char *) win_ptr->base +
			   win_ptr->disp_unit * target_disp, target_count,
			   target_datatype);
    } else {
	/* queue it up */
		     ...
		     MPID_Win * win_ptr)
{
	...
	if (HANDLE_GET_KIND(origin_datatype) == HANDLE_KIND_BUILTIN &&
	    ...
	    MPIU_ERR_CHKANDJUMP((!segp), mpi_errno, MPI_ERR_OTHER,
				"**nomem");
	    ...
	    MPIU_CHKLMEM_MALLOC(dloop_vec, DLOOP_VECTOR *,
				vec_len * sizeof(DLOOP_VECTOR), mpi_errno,
				"dloop vector");

	    MPID_Segment_pack_vector(segp, first, &last, dloop_vec,
				     &vec_len);

	    source_buf = (tmp_buf != NULL) ? tmp_buf : origin_addr;
	    target_buf =
		(char *) win_ptr->base + win_ptr->disp_unit * target_disp;
	    type = dtp->eltype;
