			...
			int tag, MPID_Comm * comm, int context_offset )
{
	...
	sreq->dev.iov[0].MPID_IOV_BUF = (MPID_IOV_BUF_CAST) 
        ((char*)sreq->dev.user_buf + dt_true_lb);
 ...
 "received rndv RTS pkt, sreq=0x%08x, rank=%d, tag=%d, context=%d, data_sz=%d",
    ...
    if (found)
    {
        ...
        if(MPIDI_CH3_RNDV_PROTOCOL_IS_READ(rts_pkt)) {
            ...
            if (mpi_errno != MPI_SUCCESS && rreq != NULL) {
                MPIU_ERR_SETANDJUMP(mpi_errno,
                        MPI_ERR_OTHER,"**ch3|rndvtransfer");
            }
        } else {
            ...
            if (rreq->dev.OnDataAvail == NULL) {
                ...
                if (rreq->dev.iov_count > 1) {
                    int k;
                    for (k = 1; k < rreq->dev.iov_count; k ++)
            ...
            /* End of OSU-MPI2 */
            mpi_errno = MPIDI_CH3_iStartMsg(vc, cts_pkt, 
					...
					MPID_Request **rreqp )
{
    ...
    *rreqp = NULL;
    ...
    MPIDI_Datatype_get_info(sreq->dev.user_count, 
            sreq->dev.datatype, dt_contig, data_sz, dt_ptr, dt_true_lb);
