		...
		MPID_Request ** request)
{
	...
	DEBUG_PRINT("MPI_Irsend, first byte (may be header) %d\n", 
	...
	if (dt_contig) {
		...
		DEBUG_PRINT("MPI_Irsend (non-contig), first byte (may be header) %d\n", ((MPIDI_CH3_Pkt_t
			*)iov[0].MPID_IOV_BUF)->type);
	...
	/* FIXME: The partner request should be set to the rts_sreq so that
	 * local
	...
	if (dt_contig) {
		...
		(MPID_IOV_BUF_CAST) ((char *) sreq->dev.user_buf +
				     dt_true_lb);
	 ...
	 * some
	 * latency.  This requires synchronization
