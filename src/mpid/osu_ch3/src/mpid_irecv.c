...
#	    endif
    ...
    rreq =
	MPIDI_CH3U_Recvq_FDU_or_AEP(rank, tag,
				    comm->context_id + context_offset,
				    &found);
    /* --BEGIN ERROR HANDLING-- */
    if (rreq == NULL) {
	...
	MPIDI_DBG_PRINTF((15, FCNAME,
			  "request found in unexpected queue"));
	...
	if (MPIDI_Request_get_msg_type(rreq) == MPIDI_REQUEST_EAGER_MSG) {
		...
		mpi_errno =
		    MPIDI_CH3_iStartMsg(vc, esa_pkt, sizeof(*esa_pkt),
					&esa_req);
		/* --BEGIN ERROR HANDLING-- */
		...
		if (HANDLE_GET_KIND(datatype) != HANDLE_KIND_BUILTIN) {
		    MPID_Datatype_get_ptr(datatype,
					  rreq->dev.datatype_ptr);
		    MPID_Datatype_add_ref(rreq->dev.datatype_ptr);
		}

	    }
	} else if (MPIDI_Request_get_msg_type(rreq) ==
	    ...
	    /* FIXME: What if the receive user buffer is not big enough to hold the
	     * data about to be cleared for sending? */
		...
		MPIDI_CH3U_Buffer_copy(sreq->dev.user_buf,
				       sreq->dev.user_count,
				       sreq->dev.datatype,
				       &sreq->status.MPI_ERROR, buf, count,
				       datatype, &data_sz,
		...
		MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_FATAL, FCNAME,
				     ...
				     "**ch3|badmsgtype",
				     "**ch3|badmsgtype %d",
				     MPIDI_Request_get_msg_type(rreq));
	    goto fn_exit;
	    /* --END ERROR HANDLING-- */
	}
    } else {
	...
	MPIDI_DBG_PRINTF((15, FCNAME,
			  "request allocated in posted queue"));
