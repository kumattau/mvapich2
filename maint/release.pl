...
my $current_ver = `svn cat ${source}/maint/version.m4 | grep MVAPICH2_VERSION_m4 | \
...
system(qq(perl -p -i -e 's/\\[MVAPICH2_RELEASE_DATE_m4\\],\\[unreleased development copy\\]/[MVAPICH2_RELEASE_DATE_m4],[$date]/g' ./src/pm/hydra/version.m4));
print("done\n");
...
chdir("${root}/${pack}-${version}");
run_cmd("rm -rf maint/config.log maint/config.status unusederr.txt src/mpe2/src/slog2sdk/doc/jumpshot-4/tex");
...
run_cmd("./configure --disable-mcast --disable-mpe --disable-fc --disable-f77 --disable-cxx");
